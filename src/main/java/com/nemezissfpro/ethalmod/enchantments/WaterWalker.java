...
public class WaterWalker extends Enchantment
{
	public WaterWalker() 
	{
		...
		this.setName("water_walker");
		this.setRegistryName(new ResourceLocation(Reference.MODID + ":water_walker"));
    ...
    public static void walkOnWater(EntityLivingBase living, World worldIn, BlockPos pos, int level)
    {
            if (living.onGround)
            {
                ...
                for (BlockPos.MutableBlockPos blockpos1 : BlockPos.getAllInBoxMutable(pos.add((double)(-f), -1.0D, (double)(-f)), pos.add((double)f, -1.0D, (double)f)))
                {
                    if (blockpos1.distanceSqToCenter(living.posX, living.posY, living.posZ) <= (double)(f * f))
                    {
                        ...
                        if (iblockstate.getMaterial() == Material.AIR)
                        {
                            ...
                            if (iblockstate1.getMaterial() == Material.WATER && (iblockstate1.getBlock() == Blocks.WATER || iblockstate1.getBlock() == Blocks.FLOWING_WATER) && ((Integer)iblockstate1.getValue(BlockLiquid.LEVEL)).intValue() == 0 && worldIn.mayPlace(ModBlocks.WATER_WALKER_BLOCK, blockpos1, false, EnumFacing.DOWN, (Entity)null))
                            {
                                worldIn.setBlockState(blockpos1, ModBlocks.WATER_WALKER_BLOCK.getDefaultState());
                                worldIn.scheduleUpdate(blockpos1.toImmutable(), ModBlocks.WATER_WALKER_BLOCK, MathHelper.getInt(living.getRNG(), 60, 120));
	...
	@SubscribeEvent
	public static void onWaterWalk(LivingEvent.LivingUpdateEvent event) {
		EntityLivingBase entity = event.getEntityLiving();
		int enchLevel = EnchantmentHelper.getMaxEnchantmentLevel(ModEnchantments.WATER_WALKER,
				entity);
		if (enchLevel != 0 && !entity.getEntityWorld().isRemote && enchLevel > 0 && hasMoved(entity))
			walkOnWater(entity, entity.getEntityWorld(), new BlockPos(entity), enchLevel);
	}
