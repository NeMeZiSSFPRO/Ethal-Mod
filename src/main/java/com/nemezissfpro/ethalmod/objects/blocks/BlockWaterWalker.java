...
public class BlockWaterWalker extends BlockBase
{
	
	public static final PropertyInteger AGE = PropertyInteger.create("age", 0, 3);

	public BlockWaterWalker(String name, Material material) 
	{
		super(name, material);
	...
	protected void turnIntoLava(World worldIn, BlockPos pos) {
		...
		worldIn.notifyNeighborsOfStateChange(pos, Blocks.WATER, false);
	}
	   ...
	   public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand)
	    {
	        ...
	        if (iblockstate.getBlock() == Blocks.WATER || iblockstate.getBlock() == Blocks.FLOWING_WATER)
	        {
	            ...
	            worldIn.playSound((EntityPlayer)null, pos, SoundEvents.BLOCK_WATERLILY_PLACE, SoundCategory.BLOCKS, 0.5F, 2.6F + (worldIn.rand.nextFloat() - worldIn.rand.nextFloat()) * 0.8F);
	 
	            if (worldIn instanceof WorldServer)
	            {
	                ((WorldServer)worldIn).spawnParticle(EnumParticleTypes.WATER_SPLASH, (double)blockpos.getX() + 0.5D, (double)blockpos.getY() + 0.25D, (double)blockpos.getZ() + 0.5D, 8, 0.5D, 0.25D, 0.5D, 0.0D);
	            }
	        }
